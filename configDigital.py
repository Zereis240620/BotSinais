...
def StopProcess(Lb1):
	...
	# Lb1.insert("end",f"Disconnect Connector")
...
def MaoFixa(codigo,Iq,Paridade,typeAccount,valorEntrada,Direcao,stopWin,stopLoss,timeFrame,Lb1,Gains,Perdas):
	...
	if config['status'] == True:
		...
		# print(Paridade, valorEntrada, Direcao, timeFrame)
		DbHelper.updateSinal(codigo)
		status, id = Iq.buy_digital_spot(str(Paridade), int(valorEntrada), str(Direcao), int(timeFrame))
		...
		Lb1.Append(f"{time} - {Paridade} - {'Compra' if Direcao == 'call' else 'Venda'} - M{timeFrame}")
		# Lb1.itemconfig("end", {'bg':'green'},fg='white')
		# Lb1.yview("end")
			...
			while True:
				...
				if status:
					if lucro > 0 :
						...
						print('Balance Inicial: ',balanceInicial['valor'])
						print('Balance Final: ',balanceAtual['value'])

						print('Loss: ',round(lucro,2))
						print(StopLoss['value'],StopL)
						...
						if float(StopLoss['value']) >= StopL:
							...
							Lb1.Append(f"Bateu o limite de perda diaria: {float(StopLoss['value'])}")
...
def Gale(codigo,Iq,par,valor_entrada,dir,time, stop_gain, stop_loss,Lb1,Gains,Perdas):
	...
	if config['status'] == True:
		...
		DbHelper.updateSinal(codigo)
		print(">>>>Url "+par,float(valor_entrada),dir,time)

		status, id = Iq.buy_digital_spot(par, int(valor_entrada),dir, int(time))
		
		timeHr = strftime("%H:%M:%S", gmtime())

		Lb1.Append("end",f"{timeHr} - {par} - {'Compra' if dir == 'call' else 'Venda'} - M{time}")
		# Lb1.itemconfig("end", {'bg':'green'},fg='white')
		# Lb1.yview("end")
...
def entradasSoro(codigo,Iq,Paridade,typeAccount,valorentrada,Direcao,stopWin,stopLoss,timeFrame,soro,Lb1,Gains,Perdas):
	...
	if config['status'] == True:
		...
		status, id = Iq.buy_digital_spot(Paridade, int(valorEntrada['valor']), Direcao, int(timeFrame))
